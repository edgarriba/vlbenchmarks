...
exp_name = 'exp_vassilis';

% image sequences
datasets_names = {'leuven', 'ubc'};
...
for p=1:numel(detector_names)
    ...
    for j = 1:numel(datasets_names)
        ...
        dataset = datasets.VggAffineDataset('Category',...
                                            char(datasets_names(j)));
        ...
        for m=1:numel(descriptor_types)
            ...
            if strcmp(descriptor_types(m), 'SIFT_CVC')
                descriptor = cvc_VlFeatSift();
            elseif strcmp(descriptor_types(m), 'SIFT')
